...
const Obfuscate = props => {
  ...
  const generateLink = () => {
    let link; // Combine email header parameters for use with email
  ...
  const reverse = content => typeof content !== 'undefined' && content.split('').reverse().join('').replace('(', ')').replace(')', '(');

  const obfuscatedStyle = { ...style,
    unicodeBidi: 'bidi-override',
    direction: humanInteraction === true || obfuscate === false || obfuscateChildren === false ? 'ltr' : 'rtl'
  };
  const renderedLink = humanInteraction === true || obfuscate === false || typeof children === 'object' || obfuscateChildren === false // Allow child elements
  ? linkProps : reverse(linkProps);
  const clickProps = Component === 'a' ? {
...
export { Obfuscate };
//# sourceMappingURL=obfuscator-preact-compiled.js.map